...
1. **輸入車手名字**(存在瀏覽器,下次自動帶入)
2. 選擇**模式 → 賽道 → 車型 → 變速箱**,出發
...
| 模式 | 規則 |
|---|---|
| ⏱ **單人計時賽** | 3 圈對抗碼表,各賽道最速單圈紀錄 |
| 🚨 **警車追逐** | 警車會 PIT 撞尾、第 2 圈增援、甩開後設路障攔截;被貼身逼停 → 出局 |
...
101 的翡翠玻璃、鈉黃路燈、中文霓虹、ACES tone mapping 下的 teal-and-amber 調色與 bloom — 以及招牌特性:**濕路面即時平面反射**。鏡像相機把發光體(霓虹/門架/尾燈)渲染到低解析 RT,以菲涅耳+粗糙度擾動混入路面 shader,每塊招牌都在路面拉出搖曳的光痕。
