...
export function makeWheels(car, positions, { radius = 0.48, width = 0.4, spokes = 7, rimEmissive = 0x76828f } = {}) {
  ...
  const rimFace = new THREE.MeshStandardMaterial({
    color: 0xf2f5fa, metalness: 0.9, roughness: 0.28,
    map: rimTex, emissiveMap: rimTex,
    emissive: rimEmissive, emissiveIntensity: 2.0, envMap, envMapIntensity: 1.2,
  });
...
export function makeHeadlights(car, { sx = 0.52, y = 0.62, z = 2.2, color = 0xd9e8ff, intensity = 75 } = {}) {
  const headlights = [];
  for (const s of [sx, -sx]) {
    const spot = new THREE.SpotLight(color, intensity, 60, 0.44, 1.0, 1.7);
    spot.position.set(s, y, z);
    const tgt = new THREE.Object3D();
    tgt.position.set(s * 0.4, 0.0, 26);
    car.add(tgt);
    spot.target = tgt;
    car.add(spot);
...
export function makeHeadlightPool({ color = 0xa9c8ea, opacity = 0.11 } = {}) {
...
// 車漆材質工廠:同色相 emissive 鎖色 + clearcoat
export function makePaint(colorHex, { metalness = 0.18, roughness = 0.42, emissiveScale = 0.5 } = {}) {
  ...
  return new THREE.MeshPhysicalMaterial({
    color: colorHex, metalness, roughness,
    clearcoat: 0.85, clearcoatRoughness: 0.2,
    emissive, emissiveIntensity: emissiveScale,
    envMap: getCarEnvTexture(), envMapIntensity: 0.9,
  });
}
