...
### 5.3 緝凶追捕 THE CHASE(規劃中,規則定案 v0.3,尚未實作)

**概念**:警車追逐的攻守互換 —— 玩家可選**當警察**(追撞小偷)或**當小偷**(開贓車逃)。是現有 `police.js` 的鏡像,可大量重用其追蹤/PIT/碰撞邏輯。

**通用規則**
- **限時追捕**:預設 90 秒倒數
- 逃逸方(小偷)車輛有 **3 格血**;有效撞擊扣 1 格,扣光 → 翻車 → 逮捕動畫
- **雙角色**(對稱勝敗):
  - **玩家當警察**:90 秒內把小偷撞掉 3 格血 → 制伏 → **勝**;時間到未制伏 → 小偷逃脫 → **敗**
  - **玩家當小偷**:90 秒內別被 AI 警察撞翻(撐過倒數)→ 逃脫成功 → **勝**;血量歸零翻車 → 被捕 → **敗**
- 車輛:警察 = 固定警車(警燈 + **小聲**警笛);小偷 = 隨機一台贓車(民車)
- HUD:對方血量 3 格 ・ 雙方距離 ・ 倒數計時
- 有效撞擊反饋:對方噴火花、打滑、車身冒煙(受損視覺)

**難度分級**(三檔差異)

| | 撞擊判定 | 追逐/逃逸 AI |
|---|---|---|
| **簡單** | 任何接觸都扣血 | AI 不積極逃竄/追撞;受傷會慌亂 |
| **中等** | 任何接觸都扣血 | 同簡單,但倒數較短或對方較快 |
| **困難** | 需一定相對速度 + 扣血後 **1.5 秒無敵**(防貼身連撞) | AI 積極逃竄/追撞:加速、鑽車縫、彎道甩尾;受傷會慌亂 |

- 「**受傷會慌亂**」三檔皆有:血量越低越容易打滑、降速、犯錯 → 越接近制伏越戲劇化,也讓最後一擊較好命中

**逮捕過場動畫**(本模式的驗收重頭戲):
翻車 → 鏡頭拉近定格 → 警車滑行停下 → 警察小人下車 → 小偷舉手就逮。程序化生成,先做基本款再打磨。

**計分與排行榜**:
- 警察版:**制伏耗時越短越強**(時間排序,越小越前)
- 小偷版:**存活時間 / 剩餘血量越高越強**
- 警察 / 小偷各自**獨立分榜**(視為 mode 維度的兩個新成員,如 `chase_cop` / `chase_thief`),與現有「模式 × 賽道 × 難度」三維分榜相容

**實作重用對照**:玩家追撃邏輯 ≈ 反向的 `police.rammer`;AI 小偷逃逸 ≈ 反向的玩家操作 + `opponents` 的走線/避讓;血量/翻車/動畫為新增。
